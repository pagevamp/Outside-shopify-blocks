...
class UpSellVariants extends HTMLElement {
  constructor() {
    super();
  }
  ...
  updateVariant(){
    const inputs = this.querySelectorAll('[data-section="recommendations-product"] input[type="radio"]');
    const productId = this.getAttribute('data-product-id');

    inputs.forEach((input) => {
      input.addEventListener('change', (event) => {
        const item = event.target;
        const variantID = item.getAttribute("data-option-value-id");
        const productForm = this.querySelector('[data-product-form-id="'+productId+'"]');

        if(productForm){
          const selectedID = productForm.querySelector('input[name="id"]');
          const itemButton = productForm.querySelector('button');

          if (selectedID)
            selectedID.value = variantID;
...
customElements.define('variant-upsell', UpSellVariants);
...
class CartRecommendations extends HTMLElement {
  ...
  initializeRecommendations(productId) {
    ...
    this.observer = new IntersectionObserver(
      (entries, observer) => {
        if (!entries[0].isIntersecting) return;
        observer.unobserve(this);
        this.loadCartRecommendations(productId);
      },
      {
        root: document.querySelector('cart-drawer-items'),
  ...
  loadCartRecommendations(productId) {
    fetch(`${this.dataset.url}&product_id=${productId}&section_id=${this.dataset.sectionId}`)
      .then((response) => response.text())
      .then((text) => {
        ...
        const html = document.createElement('div');
        html.innerHTML = htmlDocument.querySelector('[data-cart-recommendation]').innerHTML;
        // const recommendations = html.querySelector('[data-cart-recommendation]');
        this.querySelector('.swiper-wrapper').innerHTML = html.innerHTML;

        // if (recommendations?.innerHTML.trim().length) {
        //   document.querySelector('[data-cart-recommendation]').innerHTML = html.innerHTML;
        // }
        this.querySelector('.cart-drawer__collection-products').classList.remove('d-none');
        //this.sliderInterval();
      })
      .catch((e) => {
        console.error(e);
  ...
  sliderInterval() {
    const intervalId = setInterval(() => {
      if (typeof Swiper !== 'undefined') {
        clearInterval(intervalId); // Stop the interval when slider is found
        this.slider(); // Call slider initialization function
      } else {
        console.log('checking slider');
      }
    }, 200); // Check every 20ms
  }

  slider() {
    this.querySelector('.cart-drawer__collection-products').classList.remove('d-none');
    const recommendedSliders = this;
    ...
    const slideLength = parseInt(3, 10);
    const slidesPerViewValue = isNaN(slideLength)
      ? 4 // Default value if slideLength is NaN
      : slideLength === 2
      ? 2
      : slideLength === 3
      ? 3
      : slideLength === 4 || slideLength > 4
      ? 4
      : slideLength;
  ...
  updateProductHeight() {
    const productSelector = this.querySelectorAll('.js-product-calc');
    if (productSelector.length > 0) {
      productSelector.forEach((product) => {
        const cardBottomHeight = product.querySelector('.card__bottom');
        const cardTopHeight = product.querySelector('.card__top');
        const cardInnerHeight = product.querySelector('.card__inner');
        const cardContentHeight = product.querySelector('.card__content--height');
        const cardHeight =
          cardBottomHeight.clientHeight +
          cardTopHeight.clientHeight +
